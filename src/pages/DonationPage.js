...
const DonationPage = () => {
  ...
  const backendUrl =
    "https://pawfect-haven.onrender.com" || "http://localhost:5000";
  ...
  const handleSubmit = async (event) => {
    ...
    if (!donorName || !email || !amount || amount < 1) {
      setMessage(
        "Please fill in all details and ensure amount is at least ₹1."
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const orderResponse = await axios.post(
        `${backendUrl}/api/payments/createOrder`,
        {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          try {
            ...
            const verifyResponse = await axios.post(
              `${backendUrl}/api/payments/verifyPayment`,
              {
            ...
            if (verifyResponse.data.status === "success") {
              setMessage(
                "Donation successfully processed and verified! Thank you for your kindness!"
              );
        ...
        prefill: { name: donorName, email: email },
        notes: { "Donation for": "Animal Welfare", "Donor Email": email },
        theme: { color: "#3399CC" },
      };
  ...
  return (
    <div className="donation-page">
      ...
      <p className="donation-page-description">
        Your donation helps provide shelter, food, and medical care for animals
        in need.
      </p>
      <div className="donation-page-content">
        <form className="donation-page-form" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="donation-page-button"
            disabled={loading}
          >
